...
import { AuthenticatedRequest } from "../../../models/user/auth.model";
...
export const signUp = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const regSetting = await db("site_settings")
      .where("setting_key", "userRegistration")
      .first();

    const invitationToken = req.body.invitation_token;

    if (regSetting && regSetting.setting_value === "false") {
      const existingWaiting = await db("waiting_list")
        .where("email_address", normalizedEmail)
        .first();

      if (!existingWaiting) {
        await db("waiting_list").insert({
        ...
        const template = await db("email_templates")
          .where("slug", config.templates.registrationClosed)
          .where("status", 1)
          .first();

        if (template) {
          const emailContent = (template.content || "").replace("{email}", normalizedEmail);
          await sendEmail({
      ...
      apiRes.errorResponse(res, "Registration is temporarily closed. You have been added to our waiting list and will be notified once it reopens.");
      return;
    }
    ...
    // Mark as registered in waiting list if exists
    await db("waiting_list")
      .where("email_address", normalizedEmail)
      .update({
        status: "registered",
    ...
    let link = `${config.urls.userVerify}?email_token=${otpHash}&email_address=${normalizedEmail}&type=verify-email`;

    if (normalizedEmail) {
      const template = await db('email_templates')
        .where('slug', config.templates.verifyAccountUser)
        .where('status', 1)
...
export const verifyMail = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { token: accessToken, jti: accessJti } = TokenService.generateAccessToken(
      result.id,
      config.auth.secrets.user!,
      config.auth.userTokenExpire || "24h",
      sessionId
    );
    ...
    apiRes.successResponseWithData(res, USER.otpVerified, {
      ...
      refreshToken,
      sessionToken,
      expiresIn: config.auth.userTokenExpire || "24h",
      user: {
...
export const forgotPassword = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    let link = `${config.urls.userVerify}?email_token=${otpHash}&email_address=${normalizedEmail}&type=reset-password`;

    if (normalizedEmail) {
      const template = await db('email_templates')
        .where('slug', config.templates.forgotPasswordUser)
        .where('status', 1)
        .select('content', 'subject')
        .first();
...
export const getUserDetails = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const user: any = await db('users')
      ...
      .join("membership_plans", "user_memberships.membership_plan_id", "membership_plans.id")
      .join("credit_wallets", "users.id", "credit_wallets.user_id")
      .where('users.id', userId)
      .where('users.status', '!=', 2)
      .select("users.*", "user_memberships.membership_plan_id", "membership_plans.plan_type", "membership_plans.plan_name", "user_memberships.expiration_date", "credit_wallets.balance", "credit_wallets.frozen_credits")
      .first();
...
export const getTerms = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const db = getDB();
    let slug = config.cms.terms;

    switch (req.url) {
      case "/getPrivacy":
        slug = config.cms.privacyPolicy;
        break;
      case "/getCommunityGuidelines":
        slug = config.cms.communityGuidelines;
        break;
      default:
        slug = config.cms.terms;
        break;
    }
...
export const refreshToken = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const tokenSecret = adminId
      ? config.auth.secrets.admin!
      : config.auth.secrets.user!;
    const tokenExpiry = adminId
      ? config.auth.adminTokenExpire || "24h"
      : config.auth.userTokenExpire || "24h";
