...
export const joinStream = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        ...
        // Check credit balance before joining
        const userWallet = await db('credit_wallets').where('user_id', parseInt(viewerId)).first();
        const balance = userWallet ? parseFloat(userWallet.balance) : 0;
        const requiredBalance = billingRate > 0 ? billingRate : 0.1; // Min 0.1 for pyramid or package price for cam2cam

        if (balance < requiredBalance) {
            apiRes.errorResponse(res, 'Your credit balance is insufficient. Please go to the Credit Exchange to purchase credits!');
            return;
        }
...
export const pyramidRooms = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        ...
        // Get viewer counts for each room
        const viewerCounts = await db('room_sessions')
            .whereNull('session_end')
            .where('room_type', 'pyramid')
            .groupBy('pyramid_room_id')
            .select('pyramid_room_id', db.raw('COUNT(id) as count'));

        const countMap = new Map();
        viewerCounts.forEach((vc: any) => countMap.set(vc.pyramid_room_id, parseInt(vc.count)));
        ...
        const formattedRooms = pyramidRoomsList.map((room: any) => ({
            ...
            viewer_count: countMap.get(room.id) || 0,
...
export const getPyramidRoomDetails = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if (room.room_status !== 1) {
            apiRes.errorResponse(res, "Stream is offline.");
            return;
        }
        ...
        // // Add permissions and membership info
        // const userId = (req as any).user?.id || (req as any).user?._id;
        // const [privateMessageUnlocked, isPremiumMember] = await Promise.all([
        //     AccessService.hasUnlockedPrivateMessage(parseInt(userId), room.streamer_id || 0),
        //     AccessService.hasPremiumAccess(parseInt(userId))
        // ]);

        const finalRoomData = {
            ...formattedRoom,
            // permissions: {
            //     privateMessageUnlocked,
            //     isPremiumMember
            // }
        };

        apiRes.successResponseWithData(res, SUCCESS.dataFound, finalRoomData);
...
// for get streamer profile
export const getStreamerProfile = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        const db = getDB();
        ...
        // Get follower count
        const followerCountResult = await db('follows')
            .where('streamer_id', streamerId)
            .count('id as count')
            .first();
        const followerCount = followerCountResult ? Number(followerCountResult.count) : 0;
        ...
        const formattedProfile = {
            ...
            bio: streamer.bio,
            isOnline: streamer.is_online,
            email: streamer.email_address,
            stats: {
                followers: followerCount
            }
        };

        apiRes.successResponseWithData(res, SUCCESS.dataFound, formattedProfile);
